...
const GameBoard = ({ selectedEmojis, onGameWin }) => {
  ...
  useEffect(() => {
    if (!selectedEmojis || selectedEmojis.length === 0) return;

    const baseEmojis = selectedEmojis.slice(0, 8); // 8 pairs
    const allTiles = shuffleArray([...baseEmojis, ...baseEmojis]).map(
      (emoji, index) => ({
        id: index,
        emoji,
        flipped: false,
  ...
  const resetGame = () => {
    const baseEmojis = selectedEmojis.slice(0, 8);
    const allTiles = shuffleArray([...baseEmojis, ...baseEmojis]).map(
      (emoji, index) => ({
        id: index,
        emoji,
